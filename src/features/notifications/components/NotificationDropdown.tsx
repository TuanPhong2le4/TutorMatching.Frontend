...
export const NotificationDropdown: React.FC<NotificationDropdownProps> = ({
  ...
  const getElapsedTime = (dateString: string) => {
    ...
    // If the date string does not have timezone indicator (no 'Z' and no '+' or '-'), append 'Z' so it is parsed as UTC
  ...
  const getTypeDetails = (type: string, title?: string, relatedEntityType?: string | null) => {
    switch (type) {
      ...
      case 'MeetingLinkUpdated':
        return { icon: '🔗', color: '#38bdf8', tab: 'bookings' };
      case 'BookingCompleted':
        return { icon: '🎓', color: '#a855f7', tab: 'bookings' };
      case 'ReviewReceived':
